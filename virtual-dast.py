...
def net_fix(source, y, weight, mask, fast_weights, bn_vars, net, epoch):
    ...
    log(pems04_pred[mmmm.bool(), :].shape, weight.shape, weight.repeat((1, 3)).shape)
    ...
    for i, (feat, label) in enumerate(train_dataloader.get_iterator()):
        ...
        if args.models not in ['DCRNN', 'STGCN', 'HA']:
            ...
            if type == 'pretrain':
                ...
                mmmm = (th_mask_virtual.reshape((-1)))
                pems04_pred = torch.mul(pems04_pred[mmmm.bool(), :], weight.repeat((3, 1)))
                pems04_label = torch.mul(pems04_label[mmmm.bool(), :], weight.repeat((3, 1)))
