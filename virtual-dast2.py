    ...
    for i, (feat, label) in enumerate(train_dataloader.get_iterator()):
        ...
        if args.models not in ['DCRNN', 'STGCN', 'HA']:
            ...
            if type == 'pretrain':
                ...
                vc_pred = domain_classifier(shared_vc_feat, constant, Reverse)
                ...
                vc_label = 3 * torch.ones(vc_pred.shape[0]).long().to(device)
                ...
                vc_loss = domain_criterion(vc_pred, vc_label)

                domain_loss = pems04_loss + pems07_loss + pems08_loss + vc_loss
...
class DASTNets(nn.Module):
    def __init__(self, input_dim, hidden_dim, encode_dim, device, batch_size, etype, pre_len, dataset, ft_dataset,
        ...
        self.pems08_featExtractor = Extractor_N2V(input_dim, hidden_dim, encode_dim, device, batch_size, etype).to(
            device)
        self.vc_featExtractor = Extractor_N2V(input_dim, hidden_dim, encode_dim, device, batch_size, etype).to(device)
        ...
        self.shared_pems08_featExtractor = Extractor_N2V(input_dim, hidden_dim, encode_dim, device, batch_size,
                                                         etype).to(device)
        self.shared_vc_featExtractor = Extractor_N2V(input_dim, hidden_dim, encode_dim, device, batch_size, etype).to(
            device)
    ...
    def forward(self, vec_pems04, vec_pems07, vec_pems08, feat, eval, need_road=True):
        if self.dataset != self.finetune_dataset:
            if not eval:
                ...
                shared_vc_feat = self.shared_vc_featExtractor(self.vec_vc, self.adj_vc).to(self.device)
            else:
            ...
            if not eval:
                return pred, shared_pems04_feat, shared_pems07_feat, shared_pems08_feat, shared_vc_feat
            else:
                return pred
        else:
            if self.dataset == '4' or self.dataset == 'ny':
    ...
    def functional_forward(self, vec_pems04, vec_pems07, vec_pems08, feat, eval, params, bn_vars, bn_training,
                           data_set="4"):
        if self.dataset != self.finetune_dataset:
            if not eval:
...
if os.path.exists(pretrain_model_path):
    ...
    for dataset in [item for item in datasets if item not in [dataset_bak]]:
        ...
        model = DASTNets(input_dim=args.vec_dim, hidden_dim=args.hidden_dim, encode_dim=args.enc_dim,
                         ...
                         adj_pems04=adj_pems04, adj_pems07=adj_pems07, adj_pems08=adj_pems08, adj_vc=adj_virtual,
                         vec_vc=vec_virtual, vc_node=adj_virtual.shape[0]).to(device)
