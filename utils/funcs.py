...
def masked_loss(y_pred, y_true, maskp=None, weight=None):
    ...
    if maskp is not None:
        mask = maskp
    mae_loss = mae_loss * mask if weight is None else (mae_loss * torch.from_numpy(maskp).to(y_pred.device))[:, torch.from_numpy(maskp).to(y_pred.device).reshape((-1))] * weight
