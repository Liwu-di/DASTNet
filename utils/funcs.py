...
def masked_loss(y_pred, y_true, maskp=None, weight=None, maxs=2, mins=1):
    ...
    mae_loss = torch.mul(mae_loss.reshape(y_true.shape[0], -1), maskp.repeat((y_true.shape[0], 1)))
    mse_loss = torch.mul(mse_loss.reshape(y_true.shape[0], -1), maskp.repeat((y_true.shape[0], 1)))
...
def masked_loss0(y_pred, y_true):
    mask_true = (y_true > 0.01).float()
    mask_pred = (y_pred > 0.01).float()
    mask = torch.mul(mask_true, mask_pred)
    if mask.mean() < 1e-6:
        mask = torch.ones(mask.shape).type_as(mask) * torch.Tensor([1.5]).type_as(mask)
    mask /= mask.mean()
    mae_loss = torch.abs(y_pred - y_true)
    mse_loss = torch.square(y_pred - y_true)
